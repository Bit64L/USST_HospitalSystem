		...
		// ResultSet rs = db.select("select * from Doctor");
 		while(true){
 				try {
					...
					{
						try {
							dpd.setValue(rs.getString("hospitalDepartmentName"),rs.getDouble("money"));
	        ...
	        JFreeChart chart=ChartFactory.createPieChart("就诊金额",dpd,true,true,false); 
	        //可以查具体的API文档,第一个参数是标题，第二个参数是一个数据集，第三个参数表示是否显示Legend，第四个参数表示是否显示提示，第五个参数表示图中是否存在URL
	        
	        ChartFrame chartFrame=new ChartFrame("科室就诊金额",chart); 
